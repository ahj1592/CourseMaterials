...
- $\mathbf{X}_{\mathrm{1D}} \in \mathbb{R}^{T \times C}$: 전체 시계열 데이터
- $\mathrm{FFT}(\cdot)$는 고속 푸리에 변환으로 주파수 $f_i$를 찾는다
- $\mathrm{Amp}(\cdot)$: 주파수 $f_i$의 진폭을 찾는 함수
- $\mathrm{Avg}(\cdot)$: $C$차원 시계열 데이터에 대하여 진폭의 평균 계산

아래 일련의 과정을 거쳐 강도(indensity) $\mathbf{A}$를 얻습니다.


$$ \mathbf{A} = \mathrm{Avg}\Bigl( \mathrm{Amp}(\mathrm{FFT}(\mathbf{X}_{\mathrm{1D}})) \Bigr), \quad \mathbf{A} \in \mathbb{R}^T $$
...
$$\{ f_1, \cdots, f_k\} = \underset{f_* \in \{1, \cdots , [\frac{T}{2}]\}}{\mathrm{argTopK}(\mathbf{A})}, \quad p_i = \Biggl\lceil\cfrac{T}{f_i} \Biggr\rceil, \quad i \in \{ 1, \cdots, k \}$$

위 과정을 요약하면, $\mathbf{X}_{\mathrm{1D}}$로부터 FFT를 이용하여 $k$개의 유의미한 진폭($\mathbf{A}$), 주파수($f_i$), 주기($p_i$)를 얻습니다.
$$\mathbf{A}, \{f_1, \cdots, f_k\}, \{p_1, \cdots, p_k\} = \mathrm{Period}(\mathbf{X}_{\mathrm{1D}})$$
...
FFT로 얻은 $f$와 $p$를 이용하여 $\mathbf{X}_{\mathrm{1D}}$로부터 $k$개의 2D-tensor $\mathbf{X}_{\mathrm{2D}}$ 를 얻을 수 있습니다. 이때 $\mathrm{Reshape}$ 결과가 $p_i \times f_i$ 모양이 되도록 zero-padding $\mathrm{Padding}(\cdot)$이 필요합니다.
$$\mathbf{X}_{\mathrm{2D}}^i = \underset{p_i, f_i}{\mathrm{Reshape}}(\mathrm{Padding}(\mathbf{X}_{\mathrm{1D}})), \quad i \in \set{1, \cdots, k}$$



## 3.3 TimesBlock
TimesBlock 구조는 computer vision에서 자주 사용되는 ResNet의 residual way를 적용하였다. 먼저 raw data $\mathbf{X}_{\mathrm{1D}} \in \mathbf{R}^{T \times C}$를 모델 차원에 맞게 임베딩하여 $\mathbf{X}_{\mathrm{1D}}^0 \in \mathbb{R}^{T \times d_{\mathrm{model}}}$를 얻게됩니다. 
$$\mathbf{X}_{\mathrm{1D}}^0 = \mathrm{Embed}(\mathbf{X}_{\mathrm{1D}})$$
그 이후 $l$ 번째 layer마다 deep feature $\mathbf{X}_{\mathrm{1D}}^{l}$를 구한다.
$$\mathbf{X}_{\mathrm{1D}}^l = \mathrm{TimesBlock}(\mathbf{X}_{\mathrm{1D}}^{l-1}) + \mathbf{X}_{\mathrm{1D}}^{l-1}$$
...
TimesNet은 $\mathrm{Reshape}(\cdot)$로 변환한 2D-tensor를 multi-scale 2D kernel로 학습합니다. 이때 다양한 vision backbone을 이용할 수 있는데, 저자들은 parameter-efficient한 inception block을 사용했습니다. $\mathrm{Inception}(\cdot)$을 통해 표현된 $\widehat{\mathbf{{X}}}_{\mathrm{2D}}^{l, i}$은 다시 1D로 reshape하고 길이 $T$를 보존하도록 $\mathrm{Trunc}(\cdot)$로 패딩을 제거합니다.

$$
\begin{align*}
\mathbf{A}^{l-1}, \{ f_1, \cdots, f_k \}, \{ p_1, \cdots, p_k \} &= \mathrm{Period}(\mathbf{X}_{\mathrm{1D}}^{l-1}) \\
\mathbf{X}_{\mathrm{2D}}^i &= \underset{p_i, f_i}{\mathrm{Reshape}}(\mathrm{Padding}(\mathbf{X}_{\mathrm{1D}})), \quad i \in \set{1, \cdots, k} \\
\widehat{\mathbf{{X}}}_{\mathrm{2D}}^{l, i} &= \mathrm{Inception}(\mathbf{X}_{\mathrm{2D}}^{l, i}), \quad i \in \set{1, \cdots, k} \\
\widehat{\mathbf{{X}}}_{\mathrm{1D}}^{l, i}& = \mathrm{Trunc}(\underset{1, \  (p_i \times f_i)}{\mathrm{Reshape}}(\widehat{\mathbf{{X}}}_{\mathrm{2D}}^{l, i})), \quad i \in \{1, \cdots, k \} \\
\end{align*}
$$

각 $l$번째 layer를 통과한 후 $k$개의 1D-representation $\set{\widehat{\mathbf{X}}_{\mathrm{1D}}^{l, 1}, \cdots, \widehat{\mathbf{X}}_{\mathrm{1D}}^{l, k}}$을 얻습니다.
...
$$\widehat{\mathbf{A}}_{f_1}^{l-1}, \cdots, \widehat{\mathbf{A}}_{f_k}^{l-1} = \mathrm{Softmax}\left(\mathbf{A}_{f_1}^{l-1}, \cdots, \mathbf{A}_{f_k}^{l-1} \right)$$
$$\mathbf{X}_{\mathrm{1D}}^l = \sum_{i=1}^{k} \widehat{\mathbf{A}}_{f_i}^{l-1} \times \widehat{\mathbf{X}}_{\mathrm{1D}}^{l, i}$$
